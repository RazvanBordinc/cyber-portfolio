export function decryptFunction(ciphertext, base64Key) {
  ...
  for (let i = 0; i < cipherBytes.length; i++) {
    cipherBytes[i] = parseInt(ciphertext.substring(i * 2, i * 2 + 2), 16);
  }
  ...
  // Validate padding
  let validPadding = paddingLength > 0 && paddingLength <= BLOCK_SIZE;
  if (validPadding) {
  ...
  function decryptCBC(ciphertext, iv, xOrder, calcOrder, key4) {
    if (ciphertext.length % BLOCK_SIZE !== 0) {
      return 0; // Invalid ciphertext length
    }
  ...
  function feistelDecryptBlock(block, rounds, xOrder, calcOrder, key4) {
    ...
    for (let round = 0; round < rounds; round++) {
      ...
      for (let j = 0; j < half; j++) {
        ...
        const lByte = L[permIndex < half ? permIndex : permIndex - half];
